...
export default {
  ...
  methods: {
    ...
    loadGleamWidget (elem) {
      ...
      const defaultImage = 'https://js.gleam.io/images/logo.png'

      if (!elem.href.match(/(?:\/|%2f)([a-z0-9]{5})(?:\/|%2f)/i, '/')) {
        return
      }
      var loadingText = elem.getAttribute('data-loading-text') || 'Loading...'
      var key = elem.href.match(/(?:\/|%2f)([a-z0-9]{5})(?:\/|%2f)/i, '/')[1]
